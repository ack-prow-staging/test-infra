...
| Cut over | **11** — every ACK path plus `prow-build-cluster-connection`, all `automated`, prune off |
| ACK CRs | 63, all Argo CD-tracked, 0 deleting |
| Still on Flux | `prow-mirror` |
...
bootstrap, which is what matters for the hook-driven paths — `prow-charts` dependsOn them, so
they run before Prow exists and a Prow-owned schedule would be circular. To give such a path
a re-run trigger, set `selfHeal` so live drift causes a sync; only do that where the chart
owns no AWS resource. A sync requested by patching `operation` defaults to
`syncStrategy.apply`, which skips hooks and cannot be cleared that way — use `automated`.
...
| item | blocker |
|---|---|
| `prow-mirror` | Its Application has **zero** tracked resources — its only object is a Helm hook Job, which Argo CD excludes from comparison. So it is permanently Synced, no drift is possible, and nothing ever triggers a sync to run the hook. It cannot move to a Prow-owned schedule either: it mirrors the images Prow runs from, so `prow-charts` dependsOn it and that would be circular. Needs a bootstrap-safe trigger — most likely Terraform, alongside the existing `null_resource.bootstrap_prow_images_job` |
